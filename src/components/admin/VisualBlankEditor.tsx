...
export default function VisualBlankEditor({ value, onChange, placeholder, className }: VisualBlankEditorProps) {
    const editorRef = useRef<HTMLDivElement>(null);
    const [isInternalUpdate, setIsInternalUpdate] = useState(false);
    const [draggedElement, setDraggedElement] = useState<HTMLElement | null>(null);

    // Initial value setup and external sync
    useEffect(() => {
        if (isInternalUpdate) {
    ...
    const convertToHtml = (text: string) => {
        if (!text) return '';
        // Replace [______] with pill HTML
        return text.replace(/\[______\]/g,
    ...
    const handleDragStart = (e: React.DragEvent) => {
        ...
        if (target.dataset.blank === 'true') {
            ...
            // Wait a tiny bit to hide the original while dragging
            setTimeout(() => {
    ...
    // Handle dropping if items are dragged from elsewhere or internally
    const handleDrop = (e: React.DragEvent) => {
        e.preventDefault();
        if (!editorRef.current || !draggedElement) return;

        // Check if the drop target is within our editor
        ...
        if ((document as any).caretRangeFromPoint) {
            ...
            // Firefox fallback if needed, though modern FF supports caretRangeFromPoint
